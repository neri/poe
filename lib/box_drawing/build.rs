...
"
    ...
    for line in lines.iter().skip(1) {
        ...
        let description =
            format!("'{}' U+{:04X} {}", char::from_u32(unicode).unwrap(), unicode, description,);
        ...
        writeln!(os, "pub const {}: char = '\\u{{{:04x}}}';", identifier, unicode).unwrap();
    }
...
"
    ...
    for identifier in identifiers.iter() {
        let item = map_table.get(identifier).unwrap();
        writeln!(os, "            Self::{} => '\\u{{{:04x}}}',", item.identifier, item.unicode)
            .unwrap();
    }
...
"
    ...
    for identifier in identifiers.iter() {
        let item = map_table.get(identifier).unwrap();
        writeln!(os, "            Self::{} => {:#010b},", item.identifier, item.udlr).unwrap();
    }
...
"
    ...
    for (_, item) in map_table.iter() {
        items.insert(
            item.ascii_alt,
            format!("            {:#02x} => b{:?},\n", item.ascii_alt, item.ascii_fallback()),
        );
    }
    sort_and_write(os, &items);
