...
unsafe impl AcpiTable for Rsdt {
    const TABLE_ID: TableId = TableId::RSDT;
}
...
impl<'a> Iterator for RsdtTables<'a> {
    type Item = &'a AcpiHeader;

    fn next(&mut self) -> Option<Self::Item> {
        let offset = self.index * 4 + 36;
        if offset >= self.rsdt.header().len() {
            return None;
        } else {
            self.index += 1;

            Some(unsafe {
                &*(((self.rsdt as *const _ as *const c_void).add(offset) as *const u32)
