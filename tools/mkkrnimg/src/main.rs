...
impl InitRamfs {
    ...
    pub fn flush(&self, os: &mut dyn Write) -> Result<(), VirtualDiskError> {
        ...
        header.extend_from_slice(&0x0001beefu32.to_le_bytes());
        header.extend_from_slice(&((HEADER_SIZE + self.blob.len()) as u32).to_le_bytes());
        header.extend_from_slice(&(self.dir.len() as u32).to_le_bytes());
        header
            .extend_from_slice(&((HEADER_SIZE + self.blob.len() + dir.len()) as u32).to_le_bytes());
