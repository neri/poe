...
use crate::io::pci::{ConfigAccess, Ecam, Mmio};
...
impl PciHost<Ecam<Mmio>> {
    ...
    pub fn probe_ecam(tree: &fdt::DeviceTree) -> Option<Self> {
        let (node, base, size) = dt::find_map(tree, |node, map| {
        ...
        let access = Ecam::new(unsafe { Mmio::new(base, size) }, bus_min, bus_max);
...
impl<C: ConfigAccess> PciHost<C> {
    ...
    pub fn interrupt_for_device(&self, bdf: Bdf, pin: u8) -> Option<Irq> {
        if pin == 0 || pin > 4 {
            return None;
        }
    ...
    pub fn interrupt_for(&self, bdf: Bdf, pin: u8) -> Option<Irq> {
