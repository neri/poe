...
pub unsafe extern "fastcall" fn pic_handle_master_irq(irq: Irq, regs: &mut X86StackContext) {
    unsafe {
        ...
        if shared.redirect_bitmap & (1 << irq.0 as usize) != 0 {
            // Redirect to virtual 8086 mode (typically for bios)
...
impl Pic {
    ...
    pub(super) unsafe fn init(
        ...
        unsafe {
            ...
            shared.master.set_addrs(master_addr0, master_addr1);
            shared.slave.set_addrs(slave_addr0, slave_addr1);
            shared.redirect_table = redirect_table;
            shared.init_pic(icw1, slave_id, icw4_master, icw4_slave, redirect_mask);
    ...
    unsafe fn init_pic(
        ...
        self.icw4_master = icw4_master;
        self.icw4_slave = icw4_slave;
        unsafe {
            ...
            self.master
                .write_a1_bytes([Irq::BASE.0, icw3_m, icw4_master]);

            self.slave.write_a0(icw1);
            self.slave
                .write_a1_bytes([Irq::BASE.0 + 8, slave_id, icw4_slave]);
    ...
    pub(crate) unsafe fn exit() {
        unsafe {
            ...
            shared
                .master
                .write_a1_bytes([shared.redirect_table[0], icw3_m, shared.icw4_master]);

            shared.slave.write_a0(shared.icw1);
            shared.slave.write_a1_bytes([
                shared.redirect_table[8],
                shared.slave_id,
                shared.icw4_slave,
            ]);
...
impl I8259Device {
    ...
    #[inline]
    pub unsafe fn set_addrs(&mut self, a0: u16, a1: u16) {
        self.a0 = a0;
        self.a1 = a1;
    }

    #[inline]
    pub unsafe fn write_a0(&self, val: u8) {
        unsafe {
    ...
    #[inline]
    pub unsafe fn write_a1(&self, val: u8) {
        unsafe {
    ...
    #[inline]
    pub unsafe fn write_a1_bytes(&self, bytes: [u8; 3]) {
        unsafe {
            let port = IoPortWB(self.a1);
            port.write(bytes[0]);
            port.write(bytes[1]);
            port.write(bytes[2]);
        }
    ...
    #[inline]
    pub unsafe fn read_a0(&self) -> u8 {
        unsafe { IoPortRB(self.a0).read() }
    }

    #[inline]
    pub unsafe fn read_a1(&self) -> u8 {
    ...
    #[inline]
    pub unsafe fn read_imr(&self) -> u8 {
        unsafe { self.read_a1() }
    }

    #[inline]
    pub unsafe fn write_imr(&self, val: u8) {
        unsafe { self.write_a1(val) }
    }

    #[inline]
    pub unsafe fn set_enabled(&self, local_irq: u8, enabled: bool) {
        unsafe {
            let port = IoPortRWB(self.a1);
            let mut imr = port.read();
            if enabled {
                imr &= !(1 << local_irq);
            } else {
                imr |= 1 << local_irq;
